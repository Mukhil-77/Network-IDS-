"""
End-to-end training CLI (documented path: backend/scripts/train_pipeline.py).

The single-entry-point training pipeline, kept under backend/ so it lives
next to the ML package it drives: raw CIC-IDS2017 CSVs -> preprocessing ->
feature engineering (scaler + IncrementalPCA) -> model training ->
evaluation -> versioned artifact bundle under models/v{next}.
...
(scripts/train_pipeline.py at the project root is the same pipeline for
callers that prefer a top-level entry point.)

Usage:
    python -m backend.scripts.train_pipeline /path/to/raw/csv/dir
    python -m backend.scripts.train_pipeline /path/to/raw/csv/dir --models-dir ./models
"""
